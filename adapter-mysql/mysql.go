...
import (
	...
	"github.com/suifengpiao14/crud-mysql/adapter-mysql/formatters"
	"github.com/suifengpiao14/crud-mysql/adapter-mysql/internal/connection"
	"github.com/suifengpiao14/crud-mysql/adapter-mysql/statements"
			...
			for k, v := range val {
				...
				if k == 0 {
					...
					for k, v := range fields {
						v = inputFieldConvert(v)
						newFields[k] = v
					}

					key = fmt.Sprintf(`%s`, strings.Join(newFields, `"."`))
				}
...
func (adapter *MySQL) CountByRequest(req *http.Request) (countQuery string, err error) {
	...
	withCountFields := queries.Get("_with_count")
	if withCountFields != "" && countFields == "" {
		countFields = "*" // 有_with_count 参数，则一定有_count 参数
	}
	if countFields == "" {
		return
	}
...
func (adapter *MySQL) insert(db *sqlx.DB, tx *sql.Tx, SQL string, params ...interface{}) (sc adapters.Scanner) {
	...
	if config.PrestConf.MysqlInsertWithoutSelect {
		return
	}
	// Create return sql
...
// 输出列转换（前缀，驼峰、下划线转换）
func outputFieldConvert(field string) (newFiled string) {
	newFiled = field
	if config.PrestConf.MysqlColumnPrefix != "" {
		newFiled = strings.TrimLeft(newFiled, config.PrestConf.MysqlColumnPrefix)
	}
	if config.PrestConf.MysqlCamel2snake {
		newFiled = CamelString(newFiled)
	}
	return
}

// 输入列转换（前缀，驼峰、下划线转换）
func inputFieldConvert(field string) (newFiled string) {
	newFiled = field
	if config.PrestConf.MysqlCamel2snake { // 是否驼峰转下划线
		newFiled = SnakeString(newFiled)
	}
	if config.PrestConf.MysqlColumnPrefix != "" { // column有前缀，则需要添加前缀
		newFiled = fmt.Sprintf("%s%s", config.PrestConf.MysqlColumnPrefix, newFiled)
	}
	return
}
...
func rowsToJSON(rows *sql.Rows) ([]byte, error) {
	...
	for rows.Next() {
		...
		for i, col := range columns {
			newCol := outputFieldConvert(col)

			v := values[i]

			b, ok := v.([]byte)
			if ok {
				entry[newCol] = string(b)
			} else {
				entry[newCol] = v
			}
		}
